...
class NetCDFInterFaceClass():
    def __init__(self, logger=False, analysis=None, path=None):
        ...
        self.project_dynamic_data_partition = os.path.join("/home/stephen/","W4ReppProjectData/", "dynamic")
        this_path=os.path.join("/home/stephen/","W4ReppProjectData/AUG19/Cycle/CTRL/20190817/0600/obsproc/production")
    ...
    def _read_single_file_(self, single_netcdf_file):
        ...
        # make a figure
        Tincrement = [(Tcog[i+1] - Tcog[i]).seconds for i in range(len(Tcog)-1)]
        figure_name =  self.AnalysisTime.strftime("Start_time_%Y-%m-%d.png")
        fig, ax = plt.subplots(1,1,figsize=(8,8))
        ax.plot(Tincrement, label="{}".format('start'))
        plt.legend()
